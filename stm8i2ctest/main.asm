...
00103$:
	...
	btjf	0x5218, #2, 00124$
	btjf	0x5217, #2, 00103$
;	main.c: 367: for(int i = 0;i < d_size;i++)
00124$:
	clrw	x
00113$:
	ld	a, _d_size+0
...
;	main.c: 370: reg_check();
	...
	call	_reg_check
	popw	x
;	main.c: 371: while (!(I2C_SR1 & (1 << 7)) && (I2C_SR2 & (1 << 2)) && !(I2C_SR1 & (1 << 2)));
00108$:
	ld	a, 0x5217
	jrmi	00110$
	btjf	0x5218, #2, 00110$
...
;	main.c: 374: }
	addw	sp, #2
	ret
;	main.c: 376: void i2c_read(void){
;	-----------------------------------------
;	 function i2c_read
;	-----------------------------------------
_i2c_read:
	sub	sp, #2
;	main.c: 377: I2C_CR2 = I2C_CR2 | (1 << 2);
	bset	0x5211, #2
;	main.c: 378: I2C_DR = 0;
	mov	0x5216+0, #0x00
;	main.c: 379: reg_check();
	call	_reg_check
;	main.c: 380: I2C_DR = d_addr;
	mov	0x5216+0, _d_addr+0
;	main.c: 381: reg_check();
	call	_reg_check
;	main.c: 382: while (!(I2C_SR1 & (1 << 7)) && (I2C_SR2 & (1 << 2)) && !(I2C_SR1 & (1 << 2))); // Отправка адреса регистра
00103$:
	ld	a, 0x5217
	jrmi	00105$
	btjf	0x5218, #2, 00105$
...
;	main.c: 385: i2c_start();
	call	_i2c_start
;	main.c: 386: I2C_DR = (current_dev << 1) | (1 << 0);
	ld	a, _current_dev+0
	sll	a
	or	a, #0x01
	ld	0x5216, a
;	main.c: 387: reg_check();
	call	_reg_check
;	main.c: 388: while (!(I2C_SR1 & (1 << 1)) && !(I2C_SR1 & (1 << 2)) && !(I2C_SR1 & (1 << 6)));
00108$:
	btjt	0x5217, #1, 00110$
	btjt	0x5217, #2, 00110$
	btjf	0x5217, #6, 00108$
00110$:
;	main.c: 389: reg_check();
	call	_reg_check
;	main.c: 390: for(int i = 0;i < d_size;i++)
	clrw	x
00116$:
	ld	a, _d_size+0
	ld	(0x02, sp), a
	...
	cpw	x, (0x01, sp)
	jrsge	00114$
;	main.c: 392: data_buf[i] = I2C_DR;
	ldw	y, x
	addw	y, #(_data_buf+0)
	ld	a, 0x5216
	ld	(y), a
;	main.c: 393: while (!(I2C_SR1 & (1 << 6)));
00111$:
	btjf	0x5217, #6, 00111$
;	main.c: 390: for(int i = 0;i < d_size;i++)
	incw	x
...
;	main.c: 395: reg_check();
	call	_reg_check
;	main.c: 396: I2C_CR2 = I2C_CR2 & ~(1 << 2);
	ld	a, 0x5211
	and	a, #0xfb
	ld	0x5211, a
;	main.c: 397: reg_check();
	addw	sp, #2
;	main.c: 398: }
	jp	_reg_check
;	main.c: 399: void i2c_scan(void) {
;	-----------------------------------------
;	 function i2c_scan
;	-----------------------------------------
_i2c_scan:
	sub	sp, #2
;	main.c: 400: for (uint8_t addr = current_dev; addr < 127; addr++) {
	ld	a, _current_dev+0
	ld	(0x01, sp), a
	ld	(0x02, sp), a
00105$:
	...
	cp	a, #0x7f
	jrnc	00107$
;	main.c: 401: i2c_start();
	call	_i2c_start
;	main.c: 402: i2c_send_address(addr);
	ld	a, (0x02, sp)
	call	_i2c_send_address
;	main.c: 403: if (!(I2C_SR2 & (1 << 2))) { // Проверка на ACK
	btjt	0x5218, #2, 00102$
;	main.c: 405: current_dev = addr;
	ld	a, (0x01, sp)
	ld	_current_dev+0, a
;	main.c: 406: i2c_stop();
	addw	sp, #2
...
;	main.c: 409: i2c_stop();
	call	_i2c_stop
;	main.c: 410: I2C_SR2 = I2C_SR2 & ~(1 << 2); // Очистка флага ошибки
	bres	0x5218, #2
;	main.c: 400: for (uint8_t addr = current_dev; addr < 127; addr++) {
	inc	(0x02, sp)
	ld	a, (0x02, sp)
...
;	main.c: 412: }
	addw	sp, #2
	ret
;	main.c: 422: void cm_SM(void)
;	-----------------------------------------
;	 function cm_SM
;	-----------------------------------------
_cm_SM:
	sub	sp, #4
;	main.c: 424: char cur_dev[4]={0};
	clr	(0x01, sp)
	clr	(0x02, sp)
	clr	(0x03, sp)
	clr	(0x04, sp)
;	main.c: 425: convert_int_to_chars(current_dev, cur_dev);
	ldw	x, sp
	incw	x
	ld	a, _current_dev+0
	call	_convert_int_to_chars
;	main.c: 426: uart_write("SM ");
	ldw	x, #(___str_8+0)
	call	_uart_write
;	main.c: 427: uart_write(cur_dev);
	ldw	x, sp
	incw	x
	call	_uart_write
...
;	main.c: 434: }
	jp	_cm_SM
;	main.c: 435: void cm_RM(void)
;	-----------------------------------------
;	 function cm_RM
;	-----------------------------------------
_cm_RM:
;	main.c: 437: current_dev = 0;
	clr	_current_dev+0
;	main.c: 438: uart_write("RM\n");
	ldw	x, #(___str_10+0)
;	main.c: 439: }
	jp	_uart_write
;	main.c: 441: void cm_DB(void)
;	-----------------------------------------
;	 function cm_DB
;	-----------------------------------------
_cm_DB:
;	main.c: 443: status_check();
;	main.c: 444: }
	jp	_status_check
;	main.c: 446: void cm_ST(void)
;	-----------------------------------------
;	 function cm_ST
;	-----------------------------------------
_cm_ST:
;	main.c: 448: get_addr_from_buff();
	call	_get_addr_from_buff
;	main.c: 449: current_dev = d_addr;
	mov	_current_dev+0, _d_addr+0
;	main.c: 450: uart_write("ST\n");
	ldw	x, #(___str_11+0)
;	main.c: 451: }
	jp	_uart_write
;	main.c: 452: void cm_SR(void)
;	-----------------------------------------
;	 function cm_SR
;	-----------------------------------------
_cm_SR:
	sub	sp, #4
;	main.c: 454: i2c_start();
	call	_i2c_start
;	main.c: 455: i2c_send_address(current_dev);
	ld	a, _current_dev+0
	call	_i2c_send_address
;	main.c: 456: i2c_read();
	call	_i2c_read
;	main.c: 457: i2c_stop();
	call	_i2c_stop
;	main.c: 458: uart_write("SR ");
	ldw	x, #(___str_12+0)
	call	_uart_write
;	main.c: 459: convert_int_to_chars(d_addr, a);
	ldw	x, #(_a+0)
	ld	a, _d_addr+0
	call	_convert_int_to_chars
;	main.c: 460: uart_write(a);
	ldw	x, #(_a+0)
	call	_uart_write
;	main.c: 461: uart_write(" ");
	ldw	x, #(___str_13+0)
	call	_uart_write
;	main.c: 462: convert_int_to_chars(d_size, a);
	ldw	x, #(_a+0)
	ld	a, _d_size+0
	call	_convert_int_to_chars
;	main.c: 463: uart_write(a);
	ldw	x, #(_a+0)
	call	_uart_write
;	main.c: 464: for(int i = 0;i < d_size;i++)
	clrw	x
	ldw	(0x03, sp), x
00103$:
	ld	a, _d_size+0
...
;	main.c: 466: uart_write(" ");
	ldw	x, #(___str_13+0)
	call	_uart_write
;	main.c: 467: convert_int_to_chars(data_buf[i], a);
	ldw	x, (0x03, sp)
	ld	a, (_data_buf+0, x)
	ldw	x, #(_a+0)
	call	_convert_int_to_chars
...
;	main.c: 472: }
	jp	_uart_write
;	main.c: 473: void cm_SW(void)
;	-----------------------------------------
;	 function cm_SW
;	-----------------------------------------
_cm_SW:
	sub	sp, #4
;	main.c: 475: i2c_start();
	call	_i2c_start
;	main.c: 476: i2c_send_address(current_dev);
	ld	a, _current_dev+0
	call	_i2c_send_address
;	main.c: 477: i2c_write();
	call	_i2c_write
;	main.c: 478: i2c_stop();
	call	_i2c_stop
;	main.c: 479: uart_write("SW ");
	ldw	x, #(___str_14+0)
	call	_uart_write
;	main.c: 480: convert_int_to_chars(d_addr, a);
	ldw	x, #(_a+0)
	ld	a, _d_addr+0
	call	_convert_int_to_chars
;	main.c: 481: uart_write(a);
	ldw	x, #(_a+0)
	call	_uart_write
;	main.c: 482: uart_write(" ");
	ldw	x, #(___str_13+0)
	call	_uart_write
;	main.c: 483: convert_int_to_chars(d_size, a);
	ldw	x, #(_a+0)
	ld	a, _d_size+0
	call	_convert_int_to_chars
;	main.c: 484: uart_write(a);
	ldw	x, #(_a+0)
	call	_uart_write
;	main.c: 485: for(int i = 0;i < d_size;i++)
	clrw	x
	ldw	(0x03, sp), x
00103$:
	ld	a, _d_size+0
	...
	cpw	x, (0x01, sp)
	jrsge	00101$
;	main.c: 487: uart_write(" ");
	ldw	x, #(___str_13+0)
	call	_uart_write
;	main.c: 488: convert_int_to_chars(data_buf[i], a);
	ldw	x, (0x03, sp)
	ld	a, (_data_buf+0, x)
	ldw	x, #(_a+0)
	call	_convert_int_to_chars
;	main.c: 489: uart_write(a);
	ldw	x, #(_a+0)
	call	_uart_write
;	main.c: 485: for(int i = 0;i < d_size;i++)
	ldw	x, (0x03, sp)
	...
	jra	00103$
00101$:
;	main.c: 492: uart_write("\r\n");
	ldw	x, #(___str_9+0)
	addw	sp, #4
;	main.c: 493: }
	jp	_uart_write
;	main.c: 501: int data_handler(void)
;	-----------------------------------------
...
;	main.c: 503: p_size = 0;
	clr	_p_size+0
;	main.c: 504: p_bytes = 0;
	clr	_p_bytes+0
;	main.c: 505: d_addr = 0;
	clr	_d_addr+0
;	main.c: 506: d_size = 0;
	clr	_d_size+0
;	main.c: 507: memset(a, 0, sizeof(a));
	push	#0x03
	push	#0x00
	clrw	x
	pushw	x
	ldw	x, #(_a+0)
	call	_memset
;	main.c: 508: memset(data_buf, 0, sizeof(data_buf));
	push	#0x00
	push	#0x01
	clrw	x
	pushw	x
	ldw	x, #(_data_buf+0)
	call	_memset
;	main.c: 509: if(memcmp(&buffer[0],"SM",2) == 0)
	push	#0x02
	push	#0x00
	push	#<(___str_15+0)
	push	#((___str_15+0) >> 8)
...
;	main.c: 510: return 1;
	...
	ret
00102$:
;	main.c: 511: if(memcmp(&buffer[0],"SN",2) == 0)
	push	#0x02
	push	#0x00
	push	#<(___str_16+0)
	push	#((___str_16+0) >> 8)
...
;	main.c: 512: return 2;
	...
	ret
00104$:
;	main.c: 513: if(memcmp(&buffer[0],"ST",2) == 0)
	push	#0x02
	push	#0x00
	push	#<(___str_17+0)
	push	#((___str_17+0) >> 8)
...
;	main.c: 514: return 5;
	...
	ret
00106$:
;	main.c: 515: if(memcmp(&buffer[0],"RM",2) == 0)
	push	#0x02
	push	#0x00
	push	#<(___str_18+0)
	push	#((___str_18+0) >> 8)
...
;	main.c: 516: return 6;
	...
	ret
00108$:
;	main.c: 517: if(memcmp(&buffer[0],"DB",2) == 0)
	push	#0x02
	push	#0x00
	push	#<(___str_19+0)
	push	#((___str_19+0) >> 8)
...
;	main.c: 518: return 7;
	...
	ret
00110$:
;	main.c: 520: get_addr_from_buff();
	call	_get_addr_from_buff
;	main.c: 521: get_size_from_buff();
	call	_get_size_from_buff
;	main.c: 523: if(memcmp(&buffer[0],"SR",2) == 0)
	push	#0x02
	push	#0x00
	push	#<(___str_20+0)
	push	#((___str_20+0) >> 8)
...
;	main.c: 524: return 3;
	...
	ret
00112$:
;	main.c: 526: char_buffer_to_int();
	call	_char_buffer_to_int
;	main.c: 528: if(memcmp(&buffer[0],"SW",2) == 0)
	push	#0x02
	push	#0x00
	push	#<(___str_21+0)
	push	#((___str_21+0) >> 8)
...
;	main.c: 532: }
	ret
;	main.c: 534: void command_switcher(void)
;	-----------------------------------------
;	 function command_switcher
;	-----------------------------------------
_command_switcher:
	sub	sp, #4
;	main.c: 536: char ar[4]={0};
	clr	(0x01, sp)
	clr	(0x02, sp)
	clr	(0x03, sp)
	clr	(0x04, sp)
;	main.c: 538: switch(data_handler())
	call	_data_handler
	tnzw	x
	jrmi	00109$
	cpw	x, #0x0007
...
;	main.c: 562: }
	addw	sp, #4
	ret
;	main.c: 565: void main(void)
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	main.c: 567: uart_init();
	call	_uart_init
;	main.c: 568: i2c_init();
	call	_i2c_init
